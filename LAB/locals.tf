  ...
  external_identity_groups = merge([
    for _, cfg in local.raw_namespace_configs : {
      "${cfg.application.name}-s2" = {
        ad_group_name = "MS_CZ_LAB_${upper(cfg.application.name)}_S2"
        role          = "s2"
        app_name      = cfg.application.name
      }
      "${cfg.application.name}-developer" = {
        ad_group_name = "MS_CZ_LAB_${upper(cfg.application.name)}_Developer"
        role          = "developer"
        app_name      = cfg.application.name
      }
      "${cfg.application.name}-tech" = {
        ad_group_name = "MS_CZ_LAB_TECH_${upper(cfg.application.name)}_Tech"
        role          = "tech"
        app_name      = cfg.application.name
      }
    }
